            ...
            children: [
              ...
              Align(
                ...
                child: ElevatedButton.icon(
                  onPressed: () async {
                    await FirebaseFirestore.instance.collection('nutrition_supplements').add({
                      'who': who.text.trim(),
                      'what': what.text.trim(),
          ...
          child: Column(
            ...
            children: [
              ...
              Align(
                ...
                child: ElevatedButton.icon(
                  onPressed: () async {
                    await FirebaseFirestore.instance.collection('nutrition_beneficiaries').add({
                      'name': name.text.trim(),
              ...
              builder: (context, cons) {
                ...
                Widget kpiBeneficiaries = StreamBuilder<int>(
                  ...
                  builder: (context, snap) => _Kpi(
                    ...
                    icon: Icons.group,
                    color: Colors.blue,
                    onTap: () => setState(() { _underOnly = false; _section = 'beneficiaries'; }),
                    expanded: !isMobile,
                  ),
                );
                Widget kpiUnder = StreamBuilder<int>(
                  ...
                  builder: (context, snap) => _Kpi(
                    ...
                    color: Colors.orange,
                    onTap: () => setState(() { _underOnly = true; _section = 'underweight'; }),
                    expanded: !isMobile,
                  ),
                );
                Widget kpiSupp = StreamBuilder<int>(
                  stream: FirebaseFirestore.instance
                      ...
                      .snapshots()
                      .map((s) => s.docs.fold<int>(0, (acc, e) => acc + ((e.data()['qty'] ?? 0) as int))),
                  builder: (context, snap) => _Kpi(
                    label: 'Supplements Given',
                    value: '${snap.data ?? 0}',
                    icon: Icons.local_pharmacy_outlined,
                    color: Colors.green,
                    onTap: () => setState(() { _section = 'supplements'; }),
                    expanded: !isMobile,
                  ),
                );
                if (isMobile) {
                ...
                return Row(children: [kpiBeneficiaries, const SizedBox(width: 12), kpiUnder, const SizedBox(width: 12), kpiSupp]);
              },
            ),
            const SizedBox(height: 16),
            if (_section == 'all' || _section == 'beneficiaries' || _section == 'underweight')
            Container(
                ...
                children: [
                  Row(
                    ...
                    children: [
                      ...
                      FilterChip(
                        ...
                        selected: _section == 'underweight' ? true : _underOnly,
                        onSelected: (v) => setState(() { _underOnly = v; _section = v ? 'underweight' : 'beneficiaries'; }),
                      )
                    ],
                  ),
                  const SizedBox(height: 8),
                  StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
                    ...
                    builder: (context, snap) {
                      ...
                      if (docs.isEmpty) {
                        return const Text('No beneficiaries yet', style: TextStyle(color: Colors.black54));
                      }
                        ...
                        ...docs.map((doc) {
                          ...
                          return Column(children: [
                            ListTile(
                              ...
                              title: Text(name),
                              subtitle: Text('Age: $age  •  BMI: ${bmi.toStringAsFixed(1)}'),
                              trailing: Row(
                                mainAxisSize: MainAxisSize.min,
                                children: [
                                  if (uw)
                                    const Chip(label: Text('Underweight'), backgroundColor: Color(0xFFFFF3E0)),
                                  if (!isPatient)
                                    IconButton(
                                      tooltip: 'Give supplement',
                                      icon: const Icon(Icons.local_pharmacy_outlined, color: Colors.green),
                                      onPressed: () => _addSupplement(presetName: name),
                                    ),
                                ],
                              ),
                            ),
              ...
              child: const Column(
                ...
                children: [
                  Text('Recent Activities',
                      style: TextStyle(fontWeight: FontWeight.bold)),
                  SizedBox(height: 8),
                  Text('No recent activities yet', style: TextStyle(color: Colors.black54)),
                ],
              ),
            ),
            const SizedBox(height: 16),
                ...
                children: [
                  ...
                  StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
                    ...
                    builder: (context, snap) {
                      ...
                      if (docs.isEmpty) {
                        return const Text('No supplements recorded', style: TextStyle(color: Colors.black54));
                      }
                      return Column(children: [
                        ...docs.map((d) {
                          final s = d.data();
